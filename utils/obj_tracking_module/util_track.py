...
def not_tracked(image, object_, trackers, name, threshold, curr_frame_no,
    ...
    min_id = -1
    max_overlap = 0.0
    allowed_trackers = [(i, tracker) for i, tracker in enumerate(trackers) if tracker[-1] or tracker[3] < 3] 
    for i, entry in allowed_trackers:
        # if active or age < 3: #less than sampling rate, since inactive trackers can loose out on further immediate det. based on iou 
        tracker, bbox, car_no, age, feature, active = entry
        bxmin = int(bbox[0])
        bymin = int(bbox[1])
